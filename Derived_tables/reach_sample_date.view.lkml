...
inner join (select selected_list, min(percentile) middlepercentile from ${reach_ndt.SQL_TABLE_NAME} WHERE percentile>={% parameter paneldata.percentile_selection %}
AND weight>0 group by 1) b
